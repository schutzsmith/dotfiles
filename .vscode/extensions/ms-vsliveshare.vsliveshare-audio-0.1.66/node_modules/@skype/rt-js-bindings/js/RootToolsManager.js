...
var CircularBuffer = (function (_super) {
    ...
    function CircularBuffer(includeUnsafe, circularBufferMaxSize) {
        var _this = _super.call(this) || this;
    ...
    CircularBuffer.prototype.log = function (md, logId, fmt, args) {
        ...
        if (this._circularBuffer.length > this._circularBufferMaxSize) {
            this._circularBuffer.shift();
        }
    };
    CircularBuffer.prototype.visit = function (v) {
...
var BinaryLogFormatter = (function () {
    function BinaryLogFormatter() {
        ...
        // Header with flag ULOG_Base64 and ULOG_Reversed. asuming it's little endian allways...
        this._data = 'ULOG2' + String.fromCharCode(0x21) + String.fromCharCode(0x20)
            + String.fromCharCode(0x22) + String.fromCharCode(0x20);
    }
...
var RootToolsManagerImpl = (function () {
    ...
    RootToolsManagerImpl.prototype.filterMatches = function (md, logId, args, filters) {
    ...
    RootToolsManagerImpl.prototype.dumpLogBuffer = function (filter, options) {
        ...
        if (this._circularBuffer) {
            var lf_1 = new BinaryLogFormatter();
            var f_1 = filter.filter.map(function (cl) { return { component: cl.component === 'root' ? '' : cl.component, parsedLevel: cl.level }; });
            var stats_1 = { matchedLines: 0, totalLines: 0 };
            this._circularBuffer.visit(function (md, logId, args) {
                stats_1.totalLines++;
                if (_this.filterMatches(md, logId, args, f_1)) {
                    stats_1.matchedLines++;
                    lf_1.log(md.component, md.timestamp, md.level, logId, args);
                }
                return lf_1._data.length < _this._maxUploadSize;
            });
            lf_1.close();
            var data = lf_1.data();
            log.debug4("dumpLogBuffer: dumped %d of %d lines, size of payload: %d", stats_1.matchedLines, stats_1.totalLines, (data != null) ? data.length : 0);
            if (this._native) {
