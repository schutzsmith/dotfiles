...
class CordovaIosDeviceLauncher {
    ...
    static getBundleIdentifierFromPbxproj(xcodeprojFilePath) {
    ...
    static mountDeveloperImage() {
        return CordovaIosDeviceLauncher.getDiskImage()
            .then(function (path) {
            let imagemounter = child_process.spawn("ideviceimagemounter", [path, path + ".signature"]);
            let deferred = Q.defer();
            let stdout = "";
    ...
    static getDiskImage() {
        ...
        let pathInfo = promiseExec("xcrun -sdk iphoneos --show-sdk-platform-path").spread(function (stdout) {
